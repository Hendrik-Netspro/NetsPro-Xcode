...
def stream_model_response(model, prompt, token_callback: Optional[Callable[[str], None]] = None,
...
"""
    ...
    answer = clean_output(response["response"].strip())
...
"""
    ...
    with ThinkingIndicator():
        response = generate(model="mistral:latest", prompt=prompt)
...
# ===================== EXTERNAL API =====================
def handle_request(question, token_callback: Optional[Callable[[str], None]] = None,
                   done_callback: Optional[Callable[[], None]] = None,
                   stream: bool = True,
...
def cli_run(question, debug=False, stream=False):
    result = ask_jarvis(question, stream=stream, print_tokens=True)
